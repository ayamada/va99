VA=(()=>{var d=new (self.AudioContext||self.webkitAudioContext),h=d.createGain(),C=h.gain.value=.2,u,D=a=>{u?(h.disconnect(),u.disconnect()):h.disconnect();(u=a)?h.connect(a).connect(d.destination):h.connect(d.destination)},K=new (self.OfflineAudioContext||self.webkitOfflineAudioContext)(2,2,d.sampleRate),n=a=>{try{a.stop()}catch(b){}try{a.disconnect()}catch(b){}try{a.buffer=null}catch(b){}},L=a=>{var b=d.createBufferSource();b.buffer=a;b.addEventListener("ended",()=>{n(b)},{once:!0});a=d.createStereoPanner?.();
var c=d.createGain();(a?b.connect(a):b).connect(c).connect(h);b.G=c;b.P=a;return b},p=[],r=(a,b=0,c=0)=>{if(d.state=="suspended"||d.state=="interrupted")try{d.resume()}catch(q){}if(a instanceof AudioBuffer){var f=L(a);if(!c){for(c=p.length-1;0<=c;c--){var k,g;[k,g]=p[c];if(k===a||d.state=="suspended")g.G.gain.value/=2;g.buffer||p.splice(c,1)}p.push([a,f])}b||(f.start(),d.state=="suspended"&&setTimeout(()=>d.state=="suspended"&&n(f),999));return f}},e={},v=0,w=()=>{var a=e.i;a&&n(a);a=e.g;e={};if(a){var b,
...
()=>r(I,0,1)));var z={L:async a=>{var b=await fetch(a);if(!b.ok)throw Error(a);a=await b.arrayBuffer();return await K.decodeAudioData(a)},P:r,BGM:y,D:n,I:D,get V(){return C},set V(a){C=a;h&&(h.gain.value=a)},get A(){return d},VER:"va99-5.7.20251013",BCL:2};return z})();
